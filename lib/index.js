 ...
 *                                  local, link}], community: {...} }
 *   POST /dsh-restart/plugin  -> body { name, enabled } ; bundle membership
 *   POST /dsh-restart/community -> installs github:dujar/dsh-community-plugins
 ...
 *   POST /dsh-restart/relink   -> body { name } ; re-syncs the profile install
 *                                  so a clobbered link:/file: dep points at its
 *                                  checkout again
 *   POST /dsh-restart/reinstall  -> body { name, remote } ; fresh-cleans a
 *                                  local checkout (local or its own remote),
 *                                  restores its node_modules and repairs a
 *                                  broken link; remote 'plugin' instead
 *                                  re-adds from the registry (de-linking)
 ...
 * dujar/dsh-community-plugins; the local dev checkout (dsh-community-plugins)
 * is still recognized so existing installs keep working.
 */
const COMMUNITY_PACKAGES = ['dsh-community-plugins', 'dsh-community-plugins']

/** GitHub repo advertised/installed by the "More plugins" card. */
export const COMMUNITY_REPO = 'dujar/dsh-community-plugins'
...
/** Test seam: injected spawn / timeout for the checkout dependency restore. */
let depsInternals = {}
export function _setDepsInternals(value) {
  depsInternals = value || {}
}
export function _resetDepsInternals() {
  depsInternals = {}
}
...
/**
 * Health of the profile's physical node_modules entry for a link:/file:
 * dependency; null for anything else. A registry/git install can replace the
 * symlink with a real directory while the manifest still says link: — the
 * link: spec is healthy only when the entry is a symlink whose realpath
 * matches the recorded checkout (missing/dir/mismatch/dangling otherwise).
 * file: entries materialize as store symlinks that never match the spec path,
 * so any existing entry counts as healthy.
 */
export async function linkHealthOf(profile, name, spec) {
  if (typeof spec !== 'string') return null
  if (!spec.startsWith('link:') && !spec.startsWith('file:')) return null
  const isFile = spec.startsWith('file:')
  const profiles = join(dshHome(), 'profiles', profile)
  const entry = join(profiles, 'node_modules', name)
  const specPath = spec.slice(5) === '' ? null : resolve(profiles, spec.slice(5))
  let entryStat
  try {
  ...
  if (!entryStat.isSymbolicLink()) return { ok: false, reason: 'dir' }
  if (isFile) return { ok: true }
  if (!specPath) return { ok: false, reason: 'dangling' }
  try {
    const [entryReal, specReal] = await Promise.all([realpath(entry), realpath(specPath)])
    return entryReal === specReal ? { ok: true } : { ok: false, reason: 'mismatch' }
  } catch {
...
export async function collectInstalled(profile) {
  ...
  for (const name of bundles) {
    ...
    plugins.push({ name, spec, repo: repoFromSpec(spec), enabled: true, local: await localInfo(spec), link: await linkHealthOf(profile, name, spec) })
  }
  for (const [name, spec] of Object.entries(deps)) {
    if (seen.has(name)) continue
    seen.add(name)
    plugins.push({ name, spec, repo: repoFromSpec(spec), enabled: false, local: await localInfo(spec), link: await linkHealthOf(profile, name, spec) })
  }
  return plugins
}
...
// ---------------------------------------------------------------------------
// Community plugins install (advertised repo dujar/dsh-community-plugins)
// ---------------------------------------------------------------------------

/**
 * Run `dsh plugin --profile <p> <args...>` (pnpm under the hood) with the
 * captured output. DSH_BIN overrides the dsh executable (fallback: `dsh` on
 * PATH). Resolves { ok, output } or { ok:false, code, error }.
 */
export function runDshPlugin(profile, args) {
  return new Promise((resolve) => {
    ...
    let child
    try {
      child = spawnFn(bin, ['plugin', '--profile', profile].concat(args), { cwd: process.cwd(), env: process.env })
    } catch (error) {
 ...
 * `dsh plugin --profile <p> add github:dujar/dsh-community-plugins` (pnpm
...
export function installCommunity(profile) {
  return runDshPlugin(profile, ['add', 'github:' + COMMUNITY_REPO])
}

/**
 * Re-sync a profile's install (`dsh plugin --profile <p> install`): pnpm
 * re-materializes node_modules from the manifest, which re-links any
 * link:/file: dependency whose physical entry was replaced by a registry
 * copy, and rewrites a stale lockfile importer specifier.
 */
export function relinkPlugin(profile) {
  return runDshPlugin(profile, ['install'])
}

/**
 * Reinstall a plugin from the plugin origin (`dsh plugin --profile <p> add
 * <name>@latest`): deliberately replaces the link:/file: spec with the
 * registry one, so DSH loads the published release after a restart. The local
 * checkout on disk is never touched.
 */
export function reinstallFromPluginOrigin(profile, name) {
  return runDshPlugin(profile, ['add', name + '@latest'])
}
...
/** How long a checkout dependency restore may run before SIGKILL, in ms. */
const DEPS_TIMEOUT_MS = (() => {
  const value = Number(process.env.DSH_RESTART_DEPS_TIMEOUT_MS)
  return Number.isFinite(value) && value > 0 ? value : 10 * 60 * 1000
})()

/** Package-manager binaries for the checkout dependency restore (env-overridable). */
...
export async function reinstallGitCheckout(path, remote) {
  ...
  const deps = await restoreDeps(path)
  return { ok: true, output: outputs.join('\n'), deps }
}

/**
 * Restore a checkout's runtime dependencies after `git clean -fdx` wiped its
 * node_modules — Node resolves modules from a linked package's realpath, so
 * the checkout must install its own. Package manager picked from the
 * lockfile: package-lock.json -> npm, pnpm-lock.yaml -> pnpm, dependencies
 * without a lockfile -> npm, no dependencies -> skipped. Resolves
 * { status: 'installed' | 'skipped' | 'failed' } (+ reason/error).
 */
export function restoreDeps(path) {
  return (async () => {
    let manifest = null
    try {
    ...
    const has = (file) => stat(join(path, file)).then(() => true).catch(() => false)
    const [lockNpm, lockPnpm] = await Promise.all([has('package-lock.json'), has('pnpm-lock.yaml')])
    let bin = null
    if (lockNpm) bin = NPM_BIN
    else if (lockPnpm) bin = PNPM_BIN
    else if (manifest.dependencies && typeof manifest.dependencies === 'object' && Object.keys(manifest.dependencies).length > 0) bin = NPM_BIN
    if (!bin) return { status: 'skipped', reason: 'no deps' }
    return new Promise((resolve) => {
      const spawnFn = depsInternals.spawn ?? spawn
      const timeoutMs = depsInternals.timeoutMs ?? DEPS_TIMEOUT_MS
      let stdout = ''
      let stderr = ''
      let settled = false
      let child
      const fail = (error) => {
        if (settled) return
        settled = true
        resolve({ status: 'failed', error: String(error).slice(-2000) })
      }
      try {
        child = spawnFn(bin, ['install'], { cwd: path, env: process.env })
      } catch (error) {
      ...
      child.stdout?.on('data', (chunk) => { stdout += String(chunk) })
      child.stderr?.on('data', (chunk) => { stderr += String(chunk) })
      child.on('error', (error) => fail(error.message || error))
      child.on('close', (code) => {
        clearTimeout(killer)
        if (settled) return
        settled = true
        if (code === 0) resolve({ status: 'installed', output: stdout.trim() || stderr.trim() })
        else resolve({ status: 'failed', error: (stderr.trim() || stdout.trim() || `exit code ${code}`).slice(-2000) })
      })
    })
  })()
}
...
export async function apply(ctx) {
  ...
  // community plugin (github:dujar/dsh-community-plugins) into the profile.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
  ...
  // POST /dsh-restart/relink — repair a broken link:/file: dep by re-syncing
  // the profile install (pnpm re-materializes the symlink from the manifest).
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-restart/relink',
    handler: async (req, res) => {
      if (req.method !== 'POST') return sendJson(res, 405, { error: 'method not allowed' })
      if (!isTrustedRequest(req)) return sendJson(res, 403, { error: 'forbidden' })
      const body = await readJsonBody(req).catch(() => null)
      ...
      if (!name) return sendJson(res, 400, { error: 'name required' })
      if (BUILTIN_PREFIXES.some((p) => name.startsWith(p))) {
        return sendJson(res, 400, { error: 'cannot relink a builtin: ' + name })
      }
      try {
        const profile = await resolveProfile()
        const manifest = await readProfileManifest(profile)
        const spec = manifest?.dependencies?.[name]
        if (typeof spec !== 'string') return sendJson(res, 404, { ok: false, error: 'not installed: ' + name })
        if (!spec.startsWith('link:') && !spec.startsWith('file:')) {
          return sendJson(res, 400, { ok: false, error: 'not a local plugin: ' + name })
        }
        const result = await relinkPlugin(profile)
        sendJson(res, result.ok ? 200 : 500, { ok: result.ok, profile, name, ...result })
      } catch (error) {
        sendJson(res, 500, { error: 'relink failed: ' + errorMessage(error) })
      }
    },
  }), 'dsh-restart: /dsh-restart/relink route')

  // POST /dsh-restart/reinstall — fresh-clean a local plugin checkout
  // (remote 'plugin' instead re-adds from the registry, de-linking the
  // checkout), then restore its node_modules and repair a broken link.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (remote === 'plugin') {
          if (BUILTIN_PREFIXES.some((p) => name.startsWith(p))) {
            return sendJson(res, 400, { error: 'cannot reinstall a builtin: ' + name })
          }
          const manifest = await readProfileManifest(profile)
          const spec = manifest?.dependencies?.[name]
          ...
          const checkout = spec.slice(5)
          const result = await reinstallFromPluginOrigin(profile, name)
          if (!result.ok) return sendJson(res, 500, { ok: false, error: result.error })
          return sendJson(res, 200, { ok: true, name, remote, unlinked: true, checkout, output: result.output })
        }
        const manifest = await readProfileManifest(profile)
        const spec = manifest?.dependencies?.[name]
        const path = typeof spec === 'string' && (spec.startsWith('link:') || spec.startsWith('file:')) ? spec.slice(5) : null
        if (!path) return sendJson(res, 404, { ok: false, error: 'not a local plugin: ' + name })
        const result = await reinstallGitCheckout(path, remote)
        if (!result.ok) {
          if (result.invalid) return sendJson(res, 400, { ok: false, error: result.error })
          if (result.missing) return sendJson(res, 404, { ok: false, error: result.error })
          return sendJson(res, 500, { ok: false, error: result.error, step: result.step })
        }
        // A clobbered link would make the reinstall a no-op for DSH: repair it.
        let relink = { attempted: false }
        const health = await linkHealthOf(profile, name, spec)
        if (health && !health.ok) {
          const repaired = await relinkPlugin(profile)
          relink = { attempted: true, ok: repaired.ok, ...(repaired.ok ? { output: repaired.output } : { error: repaired.error }) }
        }
        sendJson(res, 200, { ok: true, name, remote, output: result.output, deps: result.deps, relink })
      } catch (error) {
